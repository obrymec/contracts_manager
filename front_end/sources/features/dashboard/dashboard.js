...
function logout () {
    ...
    if (window.SELECT) {
        ...
        // Loads login page.
        ]); set_cookie ("contracts_user", undefined, (20 / 60)); load_view ("./sources/features/sign_in/sign_in.html", "div.views", '', "Loading...");
    }
}
...
function generic_task (type, title, ready = null, drop_init = null, card_ref = null, toolbar = null) {
	...
	if (network_manager ()) {
		...
		draw_basic_widget ("./sources/utils/forms.html", 280, title, "gen-tsk", widget => {
			// For employee adding.
...
$ (() => {
    ...
    $ ("div.icons-contrat").click (() => {
		...
		$ ("div.big-title > label").text ("Contract(s)"); select_option ("div.icons-contrat", "./sources/features/get_contracts/get_contracts.html", props_to_destroyed);
    // Fixing "click" event on "employee(s)" option.
		...
		$ ("div.big-title > label").text ("Employee(s)"); select_option ("div.icons-employee", "./sources/features/get_employees/get_employees.html", props_to_destroyed);
    // Fixing "click" event on "logout" option.
	}); $ ("div.icons-logout").click (() => logout ());
    // Loads the default dashboard option.
    select_option (window.active_option, "./sources/features/get_employees/get_employees.html", [], true);
});
