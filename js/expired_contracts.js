...
function generate_html_data (card_list) {
	...
	let final_result = "<body><br/><div align = 'center'><h2><label><u>Liste des contrats terminés</u></label></h2></div>\
	...
	final_result = new DOMParser ().parseFromString (final_result, "text/html"); card_list.forEach ((card, id) => {
...
function generate_text_data (card_list) {
	...
	let text_data = String (''); card_list.forEach ((card, index) => {
...
function send_data_to_app_account_with_emailjs () {
	...
	wdp.is_closable (false); wdp.set_radius (5, 5, 5, 5); wdp.visibility (true, () => {
		// Draws the loader.
		draw_loader (new Object ({title: "Envoie des données en cours...", parent: wdp.get_content_id ()}), true);
		...
		data = new Object ({service_id: "service_zj1rqmr", template_id: "template_z1tjzgj", user_id: "gwy-tUMYeYe-bhjF8",
		    template_params: new Object ({
		    	from_name: "Simone", to_name: $ ("div.guest").attr ("title").split (':') [0], subject: "Liste des contrats terminés",
		...
		// Lauches an ajax request to emailjs service.
	    	...
	    	sign_message_box ("Les contrats terminés ont bel et bien été envoyés aux producteurs de l'application.", "green");
	    	// Closes the opened loader.
	    	wdp.visibility (false);
		}).fail (() => {
			// Warns the user about send expired contracts failed.
			sign_message_box ("Impossible d'envoyer les informations. Veuillez vérifier les identifiants.");
			// Closes the opened loader.
	    	wdp.visibility (false);
		});
	});
...
$ (() => {
	...
	sets_crud_btns_title ("un contract.", window.exp_cnt_crud); make_request ("/expired-contracts", "GET", new Object ({}), server => {
